...
type FieldType =
    Omit<IFieldProps, "onChange" | "value" | "error">
    & { name: string, validate?: (value: IFieldProps["value"]) => IFieldProps["error"] };
...
const Form: FC<IFormProps> = ({ fields, onSubmit, children, testId }) => {
    ...
    const onChangeField = useMemo(() => memoizeWith(
        String,
        (fieldName: FieldType["name"]) => (value: IFieldProps["value"]) => {
    ...
    const handleOnSubmit: FormEventHandler = useCallback((e) => {
        e.preventDefault();
